        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
                <li class="breadcrumb-item text-sm"><a class="opacity-5 text-dark"
                        href="javascript:;">{{ env('APP_NAME') }}</a></li>
            ...
            @if (auth()->user()->role)
                <h6 class="font-weight-bolder mb-0">
                    {{ __('dashboard-messages.nav_bar.' . auth()->user()->role . '_account') }}</h6>
            @endif
        </nav>
        ...
        <div class="collapse navbar-collapse mt-sm-0 mt-2 me-md-0 me-sm-4 justify-content-end" id="navbar">
            <ul class="navbar-nav justify-content-end">
                @if (auth()->user()->role === 'super-admin' || auth()->user()->role === 'admin')
                    <!-- Common Dropdown for Admin and Super-admin -->
                    ...
                    <!-- Display mode for Admin and Super-admin -->
                    <li class="nav-item d-flex align-items-center">
                        <p class="mt-2">{{ auth()->user()->role === 'admin' ? 'MultiAdminMode' : 'SuperAdminMode' }}
                        </p>
                    </li>
                @endif
                <!-- Display for User -->
                    ...
                    <li class="nav-item d-flex align-items-center">
                        <a class="btn btn-outline-dark btn-sm mb-0 me-3" href="{{ route('loans.request') }}"
                            ...
                            <i class="fa fa-piggy-bank mr-10 text-dark"
                                style="font-size: 14px; margin-right: 7px"></i>{{ __('dashboard-messages.nav_bar.request_loan_button') }}
                        </a>
                    </li>
                @endif
                <!-- Sidenav Toggler -->
